...
const Outer = styled.header`
  background: linear-gradient(red 0%, red 90px, white 90px);
...
const HeaderTitle = styled(Link)`
  ...
  background: linear-gradient(rgba(255, 0, 0, 1) 0 70%, rgba(100, 100, 100, 0));
  font-family: "Bouncy";
  src: local("Bouncy"),
    url(../fonts/Bouncy-Black-PERSONAL_USE_ONLY.otf) format("otf");
  font-weight: bold;
  border-radius: 20px;
`

const ImageTitle = styled(GatsbyImage)`
  mask-image: linear-gradient(rgba(0, 0, 0, 1) 90%, rgba(0, 0, 0, 0));
`

const Header = ({ siteTitle, heroImage }) => (
  <Outer>
    {heroImage ? ( // if heroImage exists, then show logo of heroImage instead
      <>
        <StyledLink to="/">{siteTitle}</StyledLink>
        <ImageTitle image={heroImage.gatsbyImageData} />
      </>
    ) : (
      // otherwise show the main title
      <>
        <span />
